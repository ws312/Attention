...
def test_and_generate_result(epoch_num, model_name='resnet101', img_size=320, is_multi_gpu=False):
    ...
    state_dict = torch.load('./checkpoint/' + model_name + '/Models_epoch_' + epoch_num + '.ckpt',
                            map_location=lambda storage, loc: storage.cuda())['state_dict']
    if is_multi_gpu:
    ...
    with open(os.path.join('checkpoint', model_name, model_name + '_' + str(img_size) + '_' + RESULT_FILE), 'w',
